$(() => {
  ...
  const clickChildInput = e => {
    e.target.getElementsByTagName('input')?.item(0)?.click();
  };
  ...
  $(selectors.amenityBox).change(e => {
    const amenityId = e.target.getAttribute('data-id');
  ...
  $(selectors.stateBox).change(e => {
    ...
    if (e.target.checked) {
      if (!locationsSelected.find(statePred)) {
        const stateElement = e.target.parentElement.parentElement;
  ...
  const reviewsToggleListener = e => {
    const reviewsElement = e.target.parentElement.parentElement;
